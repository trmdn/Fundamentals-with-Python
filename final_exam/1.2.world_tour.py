...
def add_stop(index, string, main_string):
    if valid_index(index):
        return main_string[:index] + string + main_string[index:]
...
def switch_stop(old_string, new_string, main_string):
...
while command != "Travel":
    ...
    if "Add" in command_type:
        ...
        main_string = switch_stop(index_or_old_string, string_or_end_index,main_string)
    print(main_string)
    command = input()
